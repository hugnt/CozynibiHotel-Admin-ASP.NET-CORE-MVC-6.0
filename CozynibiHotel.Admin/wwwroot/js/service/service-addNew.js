...
$(document).ready(async function () {
    ...
    var imgList = [];
    var iconList = [];
    var POST_RECORD = HOST + "/api/Service";
    var newRecord = {
        Images: [],
        Name: "",
        Description: "",
    ...
    async function getNewRecord() {
        //images
        newRecord.Images.push(imgList[0].name);
        newRecord.Images.push(iconList[0].name);
        ...
        await postNewRecord(newRecord, picList);
        return true;
        ...
        try {
            ...
            if (res && res.length > 0) {
                ...
                $("#success-modal-preview").on('blur', function () {
                    window.location.href = '/Admin/Menu/Service';
                });
                myModal.show();
            }
        } catch (e) {
