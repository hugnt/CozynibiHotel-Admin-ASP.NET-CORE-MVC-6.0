...
$(document).ready(async function () {
    ...
    function renderListData(res) {
                ...
                `
								...
								<td class="table-report__action w-56">
									<div class="flex justify-center items-center">
										<a class="flex items-center mr-3 btn-edit" style="cursor:pointer;" data-cate-id="${cate.id}" onclick="window.location.href='/Admin/News/NewsCategory/Edit/${cate.id}';">
        ...
        $(".btn-details").click(async function () {
            ...
            if (cate) {
                ...
                //informations
                cate.newsRate = (cate.newsRate == null || cate.newsRate == "") ? 0 : cate.newsRate;
                $('#id').val("NC0" + cate.id);
                $('#name').val(cate.name);
                $('#status').val(cate.isActive ? "Active" : "Inactive");
                $('#newsRate').val(cate.newsRate + "/5");
    ...
    $("#delete-confirmation-modal .btn-remove ").click(async function () {
        const PUT_RECORD = HOST + "/api/NewsCategory/" + RECORD_ID +"/" +true;
        var formData = new FormData();
        formData.append("newsCategoryId", RECORD_ID);
